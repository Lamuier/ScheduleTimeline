...
| 单项进行中 | 恰有一项日程处于开始（含）至结束（不含）时间段 | 显示该日程的持续通知和进度，详情末尾预告下一项（与「显示未来日程」开关无关）；Android 16 提升成功后，Status Chip 显示“进行中” | 到达结束时间，或日程被修改/删除 |
| 多项并行中 | 同时有两项或更多日程进行中 | 显示进行中数量和各项标签，进度覆盖当前活动窗口 | 所有活动结束，或剩余为单项进行中 |
