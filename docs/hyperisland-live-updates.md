...
- **启用/检测条件**（仅当全部满足才注入 HyperIsland 协议字段；否则走普通 Live Updates）：
  ...
  协议值读不到时按 OS3 候选处理；不申请白名单、不接入 MiPush。大岛按官方 9 种组合选用，不自行拼接（评估见 [hyperisland-big-island-eval.md](hyperisland-big-island-eval.md)）。
...
| 大岛模板选用评估 | `docs/hyperisland-big-island-eval.md` |
