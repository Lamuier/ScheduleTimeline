...
| 单项进行中 | 恰有一项日程处于开始（含）至结束（不含）时间段，且未被标记完成 | 显示该日程的持续通知和进度，详情末尾预告下一项时间（与「显示未来日程」开关无关）；特典可点「完成」，随后 5 秒内可「撤销」 | 到达结束时间、日程被修改/删除，或特典被标记完成 |
...
| 日程状态 | HyperIsland 内容 |
|---|---|
| 未来待办 | 标题「下一项 演·团队」，正文仅时间（当天 `14:00` / 跨天 `周六 14:00`）；大岛左「演·团队」（截断 8 字），右「后开场」 |
| 单项进行中 | 标题「演·团队」，正文 `→15:00`（有下一项时 `→15:00 · 下16:00`）；大岛左「演·团队」，右「已开场」 |
| 多项并行中 | 标题「N项进行」，正文只保留结束时间；大岛左「N项」，右「已开场」 |
