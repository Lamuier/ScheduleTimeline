...
### 新增
- 特典完成后可撤销：详情页提供「撤销完成」；通知完成动作后给出 5 秒犹豫期，可点「撤销」恢复提醒

### 修复
- 桌面小组件删除日程后仍显示旧项：列表项改为按事件 id 稳定标识，adapter intent 带上当日事件指纹强制重绑；Room 数据变化与跨日 / 时区变更都会自动刷新小组件与通知

### 变更
- 常驻通知、Live Update、小米超级岛与关键时间点提醒改为短文案（标题只放谁、正文只放何时），避免一行显示不全
