...
### 新增
- 同一天里，起止时间、类型和特典种类都相同的日程会自动合并成一条；团队名按原顺序合并，地点、场次说明和备注保留不同内容。打开应用、保存或导入时都会合并。

### 变更
- 特典列不再把重叠日程并排拆开。没有交叉的前后段各自显示自己的时间；交叉的那一段只显示合并后的一个时间段，下面列出涉及的特典，点某一条打开对应日程。演出列里彼此重叠时仍并排分栏。
