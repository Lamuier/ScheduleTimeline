...
### 修复
- 4×3 桌面小组件列表项点击在 Android 12+ 能带上事件 id 并直达编辑页（列表模板改为可变 PendingIntent，不再被系统丢掉 fill-in extras）
- 小组件深链与「新增日程」快捷方式：旋转不再重放已消费的 Intent；已在编辑页时再点新增会回到时间轴再打开空白表单，避免叠两层编辑冲掉未保存内容
- `-SetupSigning` 不再把维护者本机密钥路径写进 `AdoptKeystore` 默认值；未传路径时 `-GenerateNewKey` / `-AdoptDebugKeystore` 可正常使用
- 日期选择器按 UTC 当天 0 点读写 millis，东八区打开时不再默认偏到前一天
- 提醒通知 id 与特典「完成」动作不再对事件 id 取模 1000，避免长期使用后标错日程或误消提醒
- 删除团队候选后立即刷新通知调度，常驻标题不再残留旧团队名
- 快速切换「编辑 / 新建」时取消上一次表单加载，避免慢返回的旧事件盖掉当前表单
- 编辑已有日程时保留原来的日期，不再误用当前查看日覆盖 `dayKey`
- 特典标记完成后，进行中时段按「当前空闲」展示下一项倒计时；当前时间轴不再落在已完成卡片上
- 时间轴滚动路径去掉调试 Log；未进行中的卡片不再跑闪烁动画
- 导出时间字段固定用 `Locale.ROOT` 格式化，避免部分语言环境下 CSV 时间无法再导入

### 变更
- CSV 增加第 10 列「完成」（导出已完成特典为「是」）；旧 9 列与旧 10 列「关联演出开始时间」仍可导入

### 文档
- README / `docs/technical.md` 同步 CSV「完成」列说明

## 1.11.2 - 2026-08-23

### 变更
- Live Updates 去掉用文字/语义色区分进行中与空档进度，正文保持默认颜色
- 进入今天正在进行的时段时，当前时间轴居中显示（不再停在列表顶部）
- 今天有日程且当前时间轴滚出视野时，左下角显示「当前时间」，点击立即跳回并居中
- 通知标题与小米超级岛胶囊改用「类型字·团队名」纯文本格式（如「演·空色轨迹」），彻底规避跨进程 span 丢失，类型字与团队名清晰区分

### 修复
- 当前时间线滚出列表顶部时不再盖住「正在进行」状态条和统计条（列表默认不裁剪，后绘制的红线图层高于上方提醒）
- 进行中日程改为边框闪烁，不再画卡片左侧红进度条；时间重叠的冲突日程会一起闪
- 进行中卡片闪烁边框改用日程类型色（演出紫 / 特典玫红），与卡片底色同色系，不再使用红色
- 特典类型标签不再带「特典·」前缀：有种类时显示「前特 / 平特 / 终特」
- 日程标题旁的类型改为圆形单字标：演出 → 演、特典 → 特（形如 ①）；详情里特典仍附带前特/平特/终特
- 常驻通知锁屏隐私补齐：postLiveUpdate 由 VISIBILITY_PUBLIC 改为 VISIBILITY_PRIVATE 并提供 setPublicVersion 脱敏公开版（仅应用名 +「日程进行 * 中 / 日程即将开始」状态概要），锁屏不再暴露团队名、时间与下一项行程；解锁表面（含灵动岛 / Live Updates）展示不受影响

### 新增
- 特典详情增加「完成日程」：标记后通知不再提醒该日程（Live Update 进行中的特典也可直接点完成）
